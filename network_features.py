...
class NetworkFeatures:
    ...
    def load_network(self, network_filename: str, total_edges: int):
        ...
        with open_func(network_filename, mode = 'rt', newline = '') as f:
            next(f)
            for line in f:
    ...
    def calculate_page_rank(self, graph, damping_factor=0.85, iterations=100, weights=None) -> list[float]:
        ...
        return pagerank.fit_predict(graph.adjacency, weights)
        pass
    ...
    def get_all_network_statistics(self, graph, weights=None) -> DataFrame:
        ...
        page_rank_scores = self.calculate_page_rank(graph, weights=weights)
        authority_scores = self.calculate_hits(graph)[1]
        hub_scores = self.calculate_hits(graph)[0]
        
        df = DataFrame({"docid": list(graph.names), 
